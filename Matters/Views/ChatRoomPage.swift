    ...
    var body: some View {
        NavigationStack {
            ...
            .onAppear {
                ...
                Task {
                    print("🚀 开始懒加载聊天记录...")
                    await appState.loadRecentMessages(modelContext: modelContext, limit: 50)
...
struct ChatRoomTopBar: View {
    ...
    var body: some View {
        HStack {
            ...
            // CyberMika标题 - 霓虹渐变，AI输入时显示Typing...
            ChatRoomTypingTitle()
...
struct ChatRoomInputBar: View {
    ...
    @State private var textEditorHeight: CGFloat = 44  // 输入框动态高度
    @State private var internalFocused: Bool = false  // 内部焦点状态（用于桥接FocusState）

    // 回调：触发自动开票WebView
    var onShowAutoInvoiceWebView: ((String, CompanyInfo) -> Void)?
    
    // 计算输入框高度 - 精确考虑录音按钮占用的空间
    private func calculateTextEditorHeight(text: String) -> CGFloat {
        let font = UIFont.systemFont(ofSize: 16, weight: .semibold)
        
        // 精确计算 TextEditor 内文本的可用宽度
        let screenWidth = UIScreen.main.bounds.width
        let keyboardButtonWidth: CGFloat = internalFocused ? 38 : 0  // 键盘收回按钮（条件显示）
        let sendButtonWidth: CGFloat = 44  // 发送按钮
        let micButtonWidth: CGFloat = 44  // 录音按钮 32 + padding.trailing 12
        let outerHStackSpacing: CGFloat = 8 * 2  // 外层HStack的两个间距
        let textEditorPadding: CGFloat = 20  // TextEditor自身的 padding: leading 12 + trailing 8
        let safeMargin: CGFloat = 32  // 安全边距
        
        // TextEditor 内文本实际可用宽度
        let textWidth = screenWidth - keyboardButtonWidth - sendButtonWidth - micButtonWidth - outerHStackSpacing - textEditorPadding - safeMargin
        ...
        let maxHeight: CGFloat = 150  // 最大高度约6-7行
        let calculatedHeight = boundingRect.height + 24  // 上下padding各12，总共24
        ...
        VStack(spacing: 0) {
            ...
            HStack(spacing: 8) {
                ...
                if internalFocused {
                    Button(action: {
                        HapticFeedback.light()
                        print("⌨️ 点击键盘收回按钮，当前 internalFocused = \(internalFocused), isTextFieldFocused = \(isTextFieldFocused)")
                        ...
                        isTextFieldFocused = false
                        print("⌨️ 设置后 internalFocused = \(internalFocused), isTextFieldFocused = \(isTextFieldFocused)")
                    }) {
                ...
                HStack(alignment: .center, spacing: 0) {
                    ...
                    ZStack(alignment: .topLeading) {
                        ...
                        // TextEditor - 移除默认内边距
                        TextEditorWithoutInsets(text: $displayText, isFocused: $internalFocused)
                            .padding(.leading, 12)
                            .padding(.trailing, 8)
                            .padding(.vertical, 12)
                            ...
                            .onChange(of: internalFocused) { oldValue, newValue in
                                // 同步焦点状态到外部（单向）
                                print("🔄 InputBar: internalFocused 变化: \(oldValue) -> \(newValue)")
                                // 只在值真正不同时才更新，避免循环
                                if isTextFieldFocused != newValue {
                                    print("   -> 同步到 isTextFieldFocused")
                                ...
                                // 键盘按钮显示/隐藏时重新计算高度
                                let newHeight = calculateTextEditorHeight(text: displayText)
                                if abs(newHeight - textEditorHeight) > 1 {
                                    withAnimation(.easeOut(duration: 0.1)) {
                                        textEditorHeight = newHeight
                                    }
                            ...
                            .onChange(of: isTextFieldFocused) { oldValue, newValue in
                                // 从外部同步焦点状态（单向）
                                print("🔄 InputBar: isTextFieldFocused 变化: \(oldValue) -> \(newValue)")
                                // 只在值真正不同时才更新，避免循环
                                if internalFocused != newValue {
                                    print("   -> 同步到 internalFocused")
                    ...
                    Button(action: {
                        ...
                        ZStack {
                            Image(systemName: speechRecognizer.isRecording ? "mic.fill" : "mic")
                                .font(.system(size: 18, weight: .semibold))
                                .foregroundColor(speechRecognizer.isRecording ? Color(red: 0.6, green: 0.9, blue: 0.5) : Color.gray.opacity(0.6))
                                .opacity(isOptimizingText ? 0.3 : 1.0)
                    ...
                    .disabled(appState.isAgentTyping || isOptimizingText)
                    .padding(.trailing, 12)
                }
                .frame(minHeight: 44)
                .background(
    ...
    private func optimizeSpeechText(_ text: String) async {
        ...
        do {
            ...
            await MainActor.run {
                ...
                // 快速更新高度
                withAnimation(.easeOut(duration: 0.1)) {
                    textEditorHeight = calculateTextEditorHeight(text: optimizedText)
                }
