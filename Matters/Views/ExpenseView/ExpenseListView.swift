    ...
    var body: some View {
        ZStack {
            ...
            if showAddButton {
                VStack {
                    Spacer()
                    AddExpenseButton(showAddSheet: $showAddExpense)
                }
            }
        }
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 10) {
                ...
                Button(action: {
                    ...
                    Text("事件备注")
                        .font(.system(size: 14, weight: {
                            if case .event = filterMode { return .bold }
                            return .semibold
                        }(), design: .rounded))
                        .foregroundColor(Color.white)
                        .shadow(color: Color.black, radius: 0, x: -1, y: -1)
                ...
                ForEach(categories, id: \.self) { category in
                    Button(action: {
                        HapticFeedback.light()
                        if hasCategoryContent(category) {
                            withAnimation(.spring(response: 0.4, dampingFraction: 0.75)) {
                        ...
                        Text(category)
                            .font(.system(size: 14, weight: {
                                if case .category(let selected) = filterMode, selected == category {
                                    return .bold
                                }
                                return .semibold
                            }(), design: .rounded))
                                                ...
                                                Image(uiImage: image)
                                                    ...
                                                    .overlay(
                                                        RoundedRectangle(cornerRadius: 8)
                                                            .stroke(Color(red: 0.85, green: 1.0, blue: 0.25).opacity(0.3), lineWidth: 1)
                                                    )
                                            }
                                        }
                                    }
