...
final class TodoItem {
    ...
    // 是否全天事件（开始时间为00:00且持续时间超过12小时，或者开始结束时间相同）
    var isAllDay: Bool {
        let calendar = Calendar.current
        let startComponents = calendar.dateComponents([.hour, .minute], from: startTime)
        let duration = endTime.timeIntervalSince(startTime)
        
        // 如果开始时间是00:00且持续超过12小时，认为是全天事件
        if startComponents.hour == 0 && startComponents.minute == 0 && duration >= 12 * 60 * 60 {
            return true
        }
        
        // 如果开始和结束时间相同（没有设置具体时间），也认为是全天事件
